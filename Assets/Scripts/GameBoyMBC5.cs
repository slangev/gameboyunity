...
public class GameBoyMBC5 : GameBoyMBC {
    ...
    public GameBoyMBC5(List<byte> romMemory, List<byte> ramMemory, uint romSize, uint ramSize, bool multicart, bool battery) {
    ...
    private (uint,uint) rom_offsets(bool multicart) {
        ...
        uint upper_bank = (uint)(bank1 | (bankbit << 9));
    ...
    public void Write(ushort PC, byte data) {
        ...
        else if(PC >= 0x2000 && PC <= 0x2FFF) {
            bank1 = (byte)(data & 0xFF);
        }
        //High bit of ROM Bank Number
        else if(PC >= 0x3000 && PC <= 0x3FFF) {
            bankbit = (byte)(data & 0x1);
        }
        // RAM BANK NUMBER
        else if(PC >= 0x4000 && PC <= 0x5FFF) { 
            bank2 = (byte)(data & 0x0F);
        }
